...
def run_model(args, do_id, inPara=None):
    ...
    net = sepGAT(g, args.edge_feats, len(dl.links['count'])*2+1, in_dims, args.hidden_dim, num_classes, args.num_layers, heads, F.elu, args.dropout, args.dropout, args.slope, False, 0., decode=args.decoder)
    net.to(device)
    ...
    for name, _ in net.named_parameters():
        t = name.split('.')
        if 'fc_list' in t or 'decoder' in t or 'edge_emb' in t:
            num_all_para += 1
    ...
    for epoch in range(args.communication_round):
        ...
        for name, para in net.named_parameters():
            ...
            if 'fc_list' in t or 'decoder' in t or 'edge_emb' in t:
                ...
                # print('We aggragate {} clients for this param.'.format(count))
