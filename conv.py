...
class myGATConv(nn.Module):
    ...
    def forward(self, graph, feat, e_feat, e_count,  res_attn=None):
        with graph.local_scope():
            ...
            ee = th.cat([embed_e_feat[sum(e_count[0:i]):sum(e_count[0:i+1])] * self.attn_e for i in range(len(e_count))], 0).sum(dim=-1).unsqueeze(-1)
