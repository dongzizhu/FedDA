...
class data_loader:
    ...
    def load_links(self, name):
        ...
        with open(os.path.join(self.path, name), 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if random.random() < 0.8 and name=='link.dat.test':
                    if r_id in links['meta']:
                        continue
                # if r_id==0:
    ...
    def load_nodes(self):
        ...
        """
        nodes = {'total': 0, 'count': Counter(), 'attr': {}, 'shift': {}, 'type_dict': {}}
    ...
    def load_nodes(self):
        ...
        with open(os.path.join(self.path, 'node.dat'), 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if len(th) == 4:
                    ...
                    nodes['attr'][node_id] = node_attr
                    nodes['total'] += 1
                    nodes['type_dict'][node_id] = node_type
                elif len(th) == 3:
                    # Then this line of node doesn't have attribute
                    node_id, node_name, node_type = th
                    node_id = int(node_id)
        ...
        for i in range(len(nodes['count'])):
            attr[i] = []
        for id in range(nodes['total']):
            attr[nodes['type_dict'][id]].append(nodes['attr'][id])
        attr = list(map(np.array, attr))
